...
function loadIndividualStaffData(staff){
    ...
    if(Number.isInteger(individualStaffData.staff_rank) === true && individualStaffData.ingame_name === staff && individualStaffData.staff_rank <= 3 && individualStaffData.staff_rank >= 0 && individualStaffData.gamemode >= 0 && individualStaffData.gamemode <= 3){
    ...
    switch(discordCommand){
        ...
        case 'addstaff':
            if(!args[2] || args[3]){
                ...
                if(discordMessage.member.roles.cache.has(`${discordBotAdmin}`) === false){
        ...
        case 'restart':
            if(args[0]){
                ...
                if(discordMessage.member.roles.cache.has(`${discordBotAdmin}`) === false || discordMessage.member.roles.cache.has(`${discordBotTrusted}`) === false){
                    discordMessage.reply("```You are not allowed to run this command.```");
                } else {
